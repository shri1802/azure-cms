...
from user_app.serializers import ClaimUserSerializer, ClaimAdminSerializer, PolicyAdminSerializer
...
@api_view(['PUT'])
@permission_classes([IsAdminUser])
def admin_policy_update(request, policy_number):
    policy = get_object_or_404(Policy, policy_number=policy_number)
    serializer = PolicyAdminSerializer(policy, data=request.data)
    if serializer.is_valid():
        serializer.save()
        return Response(serializer.data)
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
@api_view(['DELETE'])
@permission_classes([IsAdminUser])
def admin_policy_delete(request, policy_number):
    try:
